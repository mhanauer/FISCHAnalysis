...
data_FISCH = read.csv("FISCH_DATA_2019-03-28_1323.csv", header = TRUE)
...
dat_FISCH[82,10] 
```
Getting rid of discontinued people use stringer r to identify them and then code them as a new variable
Get rid of anyone who has not had a session one
```{r}
library(stringr)
discontinued = str_detect(dat_FISCH$record_id, "Discontinued")

dat_FISCH$discontinued = discontinued

dim(dat_FISCH)
dat_FISCH
dat_FISCH_test1 = subset(dat_FISCH, discontinued == FALSE)
dim(dat_FISCH_test1)
sum(is.na(dat_FISCH))
...
dat_FISCH = dat_FISCH[c("record_id","colevel_1", "colevel_2", "colevel_3", "colevel_4", "colevel_5", "colevel_6","session0date", "session1date", "session2date", "session3date", "session4date", "session5date", "session6date", "gender","ethnicity", "education", "race___5")]
...
Get descriptives
```{r}
dat_FISCH_descriptive_sub = dat_FISCH[c("colevel_1", "colevel_2", "colevel_3", "colevel_4", "colevel_5", "colevel_6"), c("av_ses10", "av_ses21", "av_ses32", "av_ses43", "av_ses54", "av_ses65","gender","ethnicity", "education")]

dat_FISCH_descriptive_sub
...
dat_FISCH_long = dat_FISCH_long[c("id", "colevel_1", "av_ses10", "time")]
...
model_long_time = lmer(colevel_1 ~ av_ses10_center + (1| id),  data = dat_FISCH_long_complete)
...
### Model comparison
anova(model_null, model_long_plus, model_long_plus_re, model_long_inter_re)
