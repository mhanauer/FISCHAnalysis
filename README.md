...
Get rid of possible errors as stated on the tracking sheet
1005 data session 2 (6)
1384 data session 1 (9)
1088 data session 1 (2)
1497 data session 1 (4)
1466 data session 1 (10)
1556 data session 1 (26)
dat_FISCH
...
dat_FISCH$record_id = str_replace(dat_FISCH$record_id, " - Graduated", "")
dat_FISCH$record_id = str_replace(dat_FISCH$record_id, " - Dropout", "")

dat_FISCH$record_id = str_replace(dat_FISCH$record_id, "1311 ", "1311")


### Losing 1088 or 1497, because their session one is NA due to errors
...
Descriptives
```{r}
dat_FISCH_test  = dat_FISCH[c("record_id","colevel_1", "colevel_2", "colevel_3", "colevel_4", "colevel_5", "colevel_6","session0date", "session1date", "session2date", "session3date", "session4date", "session5date", "session6date", "gender","ethnicity", "education", "race___5", "Age")]
...
1005 data session 2 (6)
1384 data session 1 (9)
1088 data session 1 (2) (NA in data set)
1497 data session 1 (4) (NA in data set)
1466 data session 1 (10)
1556 data session 1 (26)
dat_FISCH
```{r}
...
```{r}
compmeans(dat_FISCH_long_complete$colevel_1, dat_FISCH_long_complete$intervention)
compmeans(dat_FISCH_long_complete$colevel_1, dat_FISCH_long_complete$time)
install.packages("doBy")
library(doBy)
dat_FISCH_long_complete
summaryBy(colevel_1 ~ time, data = dat_FISCH_long_complete, FUN = list(mean, max, min, median, sd))
```


Just try a multilevel model
...
model_long_time = lmer(colevel_1 ~ time + intervention + (1| id),  data = dat_FISCH_long_complete)
summary(model_long_time)

model_long_inter_time = lmer(colevel_1 ~ time*intervention + (1| id),  data = dat_FISCH_long_complete)
summary(model_long_inter_time)
...
model_long_plus_poly_avg = lmer(colevel_1 ~ poly(av_ses10_center, 2) + intervention + (1| id),  data = dat_FISCH_long_complete)
summary(model_long_plus_poly_avg)
...
### Compare the results
anova(model_long_mean, model_long_plus_poly)
R.Version()
```
Try without last unit
```{r}
dat_FISCH_long_complete_no6 = subset(dat_FISCH_long_complete, time < 5)
dim(dat_FISCH_long_complete_no6)
dim(dat_FISCH_long_complete)

model_long_mean_no6 = lmer(colevel_1 ~ intervention + (1| id),  data = dat_FISCH_long_complete_no6)
summary(model_long_mean_no6)


model_long_mean_no6_time = lmer(colevel_1 ~ intervention + poly(time,2)+ (1| id),  data = dat_FISCH_long_complete_no6)
summary(model_long_mean_no6_time)


summary(model_long_mean)

### Try without session five
dat_FISCH_long_complete_no5 = subset(dat_FISCH_long_complete, time < 4)
dim(dat_FISCH_long_complete_no5)
model_long_mean_no5 = lmer(colevel_1 ~ intervention + (1| id),  data = dat_FISCH_long_complete_no5)
summary(model_long_mean_no5)


### Try without the first session
dat_FISCH_long_complete_no_first = subset(dat_FISCH_long_complete, time > 0)
dim(dat_FISCH_long_complete_no_first)
dim(dat_FISCH_long_complete)

model_long_mean_no_first = lmer(colevel_1 ~ intervention + (1| id),  data = dat_FISCH_long_complete_no_first)
summary(model_long_mean_no_first)

model_long_mean_no_first_time = lmer(colevel_1 ~ intervention + poly(time,2)+ (1| id),  data = dat_FISCH_long_complete_no_first)
summary(model_long_mean_no_first_time)
