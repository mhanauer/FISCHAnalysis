...
```
Get rid of these people these data points
1088 – Session 1
1198 – Session 2
1497 – Session 1
```{r}
...
```{r}
head(dat_FISCH)
dat_FISCH = dat_FISCH[c("record_id","colevel_1", "colevel_2", "colevel_3", "colevel_4", "colevel_5", "colevel_6", "session1date", "session2date", "session3date", "session4date", "session5date", "session6date")]

#head(dat_FISCH)
```
Descriptives should be done with wide format, because otherwise it repeats
```{r}
...
```
###############
Try a linear change score model
```{r}
dat_FISCH_linear = dat_FISCH[c("record_id","colevel_1", "colevel_2", "colevel_3", "colevel_4", "session1date", "session2date", "session3date", "session4date")]
dim(dat_FISCH_linear)
dat_FISCH_linear_complete = na.omit(dat_FISCH_linear)
dim(dat_FISCH_linear_complete)
### Ok need to get the differnece between the average of the first three sessions and the fourth session for new variable
dat_FISCH_linear_complete_base = dat_FISCH_linear_complete[c("colevel_1", "colevel_2", "colevel_3")]
dat_FISCH_linear_complete_base = apply(dat_FISCH_linear_complete_base, 1, mean)

dat_FISCH_linear_complete_follow = dat_FISCH_linear_complete$colevel_4
dat_FISCH_linear_complete_follow = dat_FISCH_linear_complete_follow

change_score = dat_FISCH_linear_complete_follow - dat_FISCH_linear_complete_base

dat_FISCH_linear_complete$change_score = change_score
sum(is.na(dat_FISCH_linear_complete))
```
Ok now try and try and get a difference score for time
What do we want?  We want the average number of days in between sessions.  
So what is the average number of days between 1 and 2; 2 and 3; 3 and 4
Then average those averages for each person?
Average number of days between consecutive sessions
```{r}
library(lubridate)

dat_FISCH_linear_complete_date = dat_FISCH_linear_complete[c("session1date", "session2date", "session3date", "session4date")]

dat_FISCH_linear_complete_date$session1date = mdy(dat_FISCH_linear_complete_date$session1date)

dat_FISCH_linear_complete_date$session2date = mdy(dat_FISCH_linear_complete_date$session2date)

dat_FISCH_linear_complete_date$session3date = mdy(dat_FISCH_linear_complete_date$session3date)

dat_FISCH_linear_complete_date$session4date = mdy(dat_FISCH_linear_complete_date$session4date)

av_ses12 = dat_FISCH_linear_complete_date[c("session1date", "session2date")]
av_ses12 = dat_FISCH_linear_complete_date$session2date-dat_FISCH_linear_complete_date$session1date


av_ses23 = dat_FISCH_linear_complete_date[c("session2date", "session3date")]
av_ses23 = dat_FISCH_linear_complete_date$session3date-dat_FISCH_linear_complete_date$session2date

av_ses34 = dat_FISCH_linear_complete_date[c("session3date", "session4date")]
av_ses34 = dat_FISCH_linear_complete_date$session4date-dat_FISCH_linear_complete_date$session3date


ave_con_ses = data.frame(av_ses12 = as.factor(av_ses12), av_ses23 = as.factor(av_ses23), av_ses34 = as.factor(av_ses34))
write.csv(ave_con_ses, "ave_con_ses.csv", row.names = FALSE)
ave_con_ses = read.csv("ave_con_ses.csv", header = TRUE)
ave_con_ses = apply(ave_con_ses, 1, mean)

dat_FISCH_linear_complete$ave_con_ses =  ave_con_ses
```

Ok now try the linear change model
```{r}
hist(dat_FISCH_linear_complete$change_score)
describe(dat_FISCH_linear_complete$change_score)


model_change_score = lm(change_score ~ ave_con_ses , data =  dat_FISCH_linear_complete)
summary(model_change_score)
```
...
model_long_plus = lmer(colevel_1 ~ time+intervention + (1| id),  data = dat_FISCH_long_complete)
summary(model_long_plus)

model_long_time = lmer(colevel_1 ~ time + (1| id),  data = dat_FISCH_long_complete)
summary(model_long_time)
...
### Try random slopes 
model_long_plus_re = lmer(colevel_1 ~ time+intervention + (time| id),  data = dat_FISCH_long_complete)
summary(model_long_plus_re)

### Model comparison
anova(model_null,model_long_mean, model_long_plus, model_long_plus_re)
...
brms_model_plus_slopes = brm(colevel_1_stand ~ intervention + (time|id), data = dat_FISCH_long_complete)
...
brms_model_plus = brm(colevel_1_stand ~ time+intervention + (1|id), data = dat_FISCH_long_complete)
summary(brms_model_plus)

brms_model_ranslopes = brm(colevel_1_stand ~ time + intervention + (time|id), data = dat_FISCH_long_complete)
summary(brms_model_ranslopes)

brms_model_ranslopes_prior = brm(colevel_1_stand ~ time + intervention + (time|id), data = dat_FISCH_long_complete, prior = set_prior("student_t(1,-.20,.10)", class = "b", coef = "intervention"))
summary(brms_model_ranslopes_prior)
...
```
##############
Try multilevel model with time as factor instead of just a number
